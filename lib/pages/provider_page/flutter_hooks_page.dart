...
class FlutterHooksPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context,WidgetRef ref) {
    ...
    // `StatefulWidget` の `initState` のように初回ビルドのみ実行させたい処理を指定できる
    // fetchData();
    // 戻り値として関数を返す。この関数は dispose されたときに実行されるため、
    // `StatefulWidget` の `onDispose` と同じように利用できる
...
/// Flutter Hooks 
/// flutter_hooks に共通する useXxx() というメソッドは、 StatefulWidget や StatelessWidget では使用できない
/// Hookメソッドを使用するには、 HookConsumerWidget または HookConsumer を使う
/// 
/// 
/// HookConsumerWidget 
/// - build メソッドで、 useXxx メソッドと WidgetRef が使用できるWidget。 StatefulWidget や StatelessWidget の代わりに使う。
/// HookConsumer
/// - Consumer Widgetと同じく、囲った範囲内で useXxx メソッドと WidgetRef が利用できるようになる。
///   StatefulHookWidget ※ v1.0.0で削除されました
///   hooks が使用できる StatefulWidget [3]。
///
///
///
///